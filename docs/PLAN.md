...
- [x] `go build ./...` produces the `sssh` binary with no errors.
- [x] Run in a directory resolves and logs the absolute project path.
- [x] VM, tunnel, front-door, and registry managers exist as interfaces with no-op
      implementations wired through the top-level context.
- [x] SIGINT/SIGTERM triggers ordered teardown of every registered manager; the
...
**Base**
- [x] The sandbox `nixosConfiguration` builds from **sssh's own flake** and provides the
...
- [x] The VM boots via QEMU/KVM and sshd is reachable only on the host-only NIC
      (not on any public or LAN interface).
- [x] The VM manager blocks until sshd accepts a connection, with a bounded timeout
      that returns a clear error on failure.
- [x] A `pairing` tmux session exists in the VM; `ssh <vm> tmux attach -t pairing`
      from the host attaches to it.

**Owner environment**
- [x] The sandbox imports the owner's host program modules (editor, shell, tmux) and
      only those — never the host's bootloader, hardware, users, or secrets.
- [x] The shared session presents the owner's real editor/shell/tmux configuration,
      not a generic default.

**Project toolchain**
- [x] A project with a flake `devShell` boots its panes inside that shell (`nix develop`
      / a `use flake` `.envrc`); its compilers and LSPs are on `$PATH`.
- [x] The host Nix store is shared into the VM, so an already-built dev environment
      resolves without a rebuild; only uncached inputs build at session start.
- [x] A project with no flake falls back to the base tools and still boots.

## M2 — Local shared session

**Goal:** the core pairing loop, on the LAN, no internet yet.

- [x] The front door listens on a loopback/LAN address that is logged at startup.
- [x] A stock `ssh` client connecting over LAN lands in the VM `pairing` tmux, never
      a host shell.
- [x] Two simultaneous connections see mirrored, read/write tmux content.
- [x] Client window resize propagates to the tmux pane.
- [x] The registry tracks owner presence and the active set; disconnect removes the
      entry.
...
- [x] On connect a guest is prompted for a username, then an SSN.
- [x] A username already in `active`/`pending` is rejected and the guest is reprompted.
- [x] The request lands in `pending` with username, SSN, remote address, and a
      buffered decision channel.
- [x] The handler blocks until `Resolve(id, …)` or ctx cancel: accept bridges into
      tmux, decline disconnects with a message.
- [x] A guest disconnecting or timing out while pending drops the request and frees
      the username.
...
- [x] The launching terminal renders the control plane with connect strings and live
      connected/waiting counts.
- [x] A new pending request appears ambiently (no focus steal) with a soft bell;
      Terminal B (the editor) is unaffected.
- [x] `a` / `d` / `x` accept, decline, and kick the selected user and reflect in the
      registry within the session.
- [x] Kick closes the target's bridge and PTY and frees the username; the guest can
      reconnect as a fresh pending request.
- [x] Guests never see the control plane — it is never rendered into the VM tmux.
...
- [x] Owner disconnect (detach, quit, or simulated network drop) disconnects all
      guests and tears down the VM, tunnel, and session.
- [x] Guest drop removes it from `active`, frees the username, and leaves the session
      running for others.
- [x] VM-boot failure aborts startup before any tunnel is opened.
- [x] Teardown destroys the VM; nothing is retained (the project lives on the host).
- [x] Every row of DESIGN §10 has a corresponding passing test or manual verification.

## M8 — Polish (open questions)

**Goal:** resolve the §11 open questions that remain.

- [x] The front door presents a persistent SSH host key across sessions (no host-key
      warning churn for repeat guests).
- [x] A pending request expires after a configurable timeout and notifies the guest.
- [x] Each resolved open question in §11 is reflected back into `DESIGN.md`.
